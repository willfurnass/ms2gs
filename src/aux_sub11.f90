...
! call get_environment_variable("nn",wc)
...
! ULL: pos and allpos must be ordered
 integer(i8) :: pos(:), allpos(:), i, ipos
 dimension   :: iindex(size(pos))
 iindex(1:size(pos))=0
 ipos = 1
 do i=1, size(allpos)
    if(pos(ipos)==allpos(i)) then
       iindex(ipos)=i
       if(ipos==size(pos)) EXIT
       ipos=ipos+1
    endif
 enddo
...
! counts no. fields in file
 ...
 if(ex) then
    call system('head -n1 ' // infile // "| awk '{print NF}'  >"//ofile)
...
! counts file lines; removes blank end line if it exists
 ...
 if(ex) then
    ...
    !call system("gsed '${/^$/d}' "//infile//' | wc -l >'//ofile)
    call system("awk 'NR > 1{print t} {t = $0}END{if (NF) print }' "//infile//' | wc -l >'//ofile)
    open(ioin,file=ofile)
    read(ioin,*) wc
    close (ioin, status='delete')
 endif
...
!------------------------
 subroutine initia_random (seed0)
!------------------------
